...
export class ProjectService {
    ...
    onRefreshProject(): boolean {
        this.storage.getStorageProject().subscribe(prj => {
            ...
            console.error('OVISU onRefreshProject error', err);
        });
        return true;
    }
    ...
    private load() {
        this.storage.getStorageProject().subscribe(prj => {
            ...
            console.error('OVISU load error', err);
        });
    }
    ...
    private notifySaveError(err: any) {
        console.error('OVISU notifySaveError error', err);
        let msg = null;
        this.translateService.get('msg.project-save-error').subscribe((txt: string) => { msg = txt; });
        if (err.status === 401) {
    ...
    private notifyServerError() {
        console.error('OVISU notifyServerError error');
        let msg = null;
    ...
    private notifyError(msgCode: string) {
        ...
        if (msgCode) {
            console.error(`OVISU Error: ${msgCode}`);
